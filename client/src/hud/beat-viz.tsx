  ...
  useEffect(() => {
    if (containerRef.current) {
      const currentBeatPosition =
        (beatInfo.beat + beatInfo.offset) * beatWidthPx;
      ...
      >
        {Array.from({ length: totalBeats }).map((_, i) => {
          ...
          return (
            <div
              key={`beat-${beatNumber}`}
              className={`relative rounded border border-gray-600 overflow-hidden mr-0.5 flex-shrink-0 ${
                isCurrent ? "bg-yellow-400" : "bg-gray-800"
              }`}
              style={{
                width: beatWidthPx,
                height: 80,
